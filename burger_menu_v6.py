...
if __name__ == "__main__":
    ...
    while run_program:
        ...
        # Component 6: Delete
            ...
            delete_name = easygui.enterbox("Enter the name of the combo you want to delete:", "Delete Combo")
            
            if delete_name:
                delete_key = delete_name.title()
                
                if delete_key in current_menu:
                    # Double-checking with the user for safety
                    confirm = easygui.buttonbox(f"Are you sure you want to delete {delete_key}?", 
                                               "Confirm Delete", ["Yes", "No"])
                    
                    if confirm == "Yes":
                        del current_menu[delete_key]
                        easygui.msgbox(f"{delete_key} has been removed from the menu.")
                    else:
                        easygui.msgbox("Delete cancelled.")
                else:
                    easygui.msgbox(f"'{delete_name}' not found on the menu.")
